...
// Function to update the scroller indicator width based on scroll position
function updateScrollIndicator() {
    ...
    // Set the width of the scroller-indicator based on the scroll percentage
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!carousel) return; // Exit if the specific carousel is not found
    ...
    // Function to update both progress bar and button state
    function updateCarouselUI() {
        ...
        // Disable the back button if on the first item, enable otherwise
        if (activeIndex === 0) {
    ...
    // Initialize UI state: set progress bar and disable back button if on the first slide
    updateCarouselUI();

    // Attach event listener to update carousel UI on slide change
